...
local _ = LibStub("AceAddon-3.0"):NewAddon(addon, addonName, "AceConsole-3.0", "AceEvent-3.0")
...
function addon:READY_CHECK()
    StaticPopupDialogs["REBUFF_PRINT"] = {
        ...
        button1 = L["YES"],
        button2 = L["NO"],
        OnAccept = function() addon.broadcast:send() end,
        timeout = 0,
        whileDead = true,
        hideOnEscape = true,
        preferredIndex = 3
    ...
    if (addon.db.profile.options.readyDirect) then addon.broadcast:send() end
end
